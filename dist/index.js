 ...
 * Update usage record
 */
function updateUsageRecord(ctx, record) {
    const today = new Date().toDateString();
    const usage = ctx.memory.get('zigma_usage') || {};
    if (!usage[today])
        usage[today] = { signalsRequested: 0, walletAnalyses: 0 };
 ...
 */
async function handleAlpha(ctx, params) {
    try {
...
Connect wallet: \`zigma connect 0x...\`
      `.trim();
        ...
        // Enforce limit
        const effectiveLimit = config.limit === -1
        ...
        const signals = await zigmaFetch(`/api/v1/signals?${query}`, z.array(SignalSchema));
 ...
 */
async function handleConnect(ctx, params) {
    try {
 ...
 */
async function handleAnalyze(_ctx, params) {
    try {
        const marketId = extractMarketId(params.market);
        validateMarketId(marketId);
        const analysis = await zigmaFetch(`/api/v1/market/${encodeURIComponent(marketId)}/analysis`, MarketAnalysisSchema);
 ...
 */
async function handleTrack(ctx, params) {
    try {
        const tier = await checkTokenTier(ctx);
        const config = TIER_LIMITS[tier];
...
• \`zigma wallet [address]\` - Connect your wallet
      `.trim();
        ...
        const statuses = await Promise.all(tracked.map(async (t) => {
            try {
                const analysis = await zigmaFetch(`/api/v1/market/${t.marketId}/analysis`, MarketAnalysisSchema);
                return { ...t, analysis };
            }
            catch {
 ...
 */
async function handleWallet(ctx, params) {
    try {
        const tier = await checkTokenTier(ctx);
        const config = TIER_LIMITS[tier];
        const usage = getUsageRecord(ctx);
...
Connect wallet: \`zigma connect 0x...\`
      `.trim();
        ...
        validateWalletAddress(params.address);
        const analysis = await zigmaFetch(`/api/v1/wallet/${params.address}`, WalletAnalysisSchema);
        // Update usage
...
Connect wallet: \`zigma connect 0x...\`
      `.trim();
        ...
        const opportunities = await zigmaFetch('/api/v1/arbitrage', z.array(ArbitrageOpportunitySchema));
 ...
 */
async function autoPostDailySignals(ctx) {
    try {
        // Check if we already posted today
        const lastPost = ctx.memory.get('zigma_last_daily_post');
        const today = new Date().toDateString();
        if (lastPost === today)
            return;
        // Get top 3 signals
        const signals = await zigmaFetch(`/api/v1/signals?limit=3&minEdge=0.03`, z.array(SignalSchema));
        if (signals && signals.length > 0) {
 ...
 */
async function checkResolvedSignals(ctx) {
    try {
        // Get tracked markets that might have resolved
        const tracked = ctx.memory.get('zigma_tracked') || [];
        const postedOutcomes = ctx.memory.get('zigma_posted_outcomes') || [];
        for (const market of tracked) {
            if (postedOutcomes.includes(market.marketId))
                continue;
            try {
                await zigmaFetch(`/api/v1/market/${market.marketId}/analysis`, MarketAnalysisSchema);
 ...
 */
async function checkMoltbookComments(ctx) {
    try {
        const myPosts = await getMyPosts(5);
        const repliedComments = ctx.memory.get('zigma_replied_comments') || [];
 ...
 * Heartbeat handler
 */
async function handleHeartbeat(ctx) {
    try {
        // Auto-post daily signals to community
        ...
        for (const market of tracked) {
            try {
                const analysis = await zigmaFetch(`/api/v1/market/${market.marketId}/analysis`, MarketAnalysisSchema);
...
Reply "zigma analyze ${market.marketId}" for full analysis
        ...
        try {
            const signals = await zigmaFetch(`/api/v1/signals?limit=1&minEdge=${GLOBAL_LIMITS.STRONG_SIGNAL_MIN_EDGE}`, z.array(SignalSchema));
 ...
 */
async function handleLeaderboard(_ctx) {
    try {
        // Fetch leaderboard data from API
        const leaderboard = await zigmaFetch('/api/v1/leaderboard', z.array(z.object({
            agent: z.string(),
            pnl: z.number(),
            winRate: z.number(),
            trades: z.number(),
 ...
 */
async function handleShare(ctx, params) {
    try {
        // Get recent signals
        const signals = await zigmaFetch(`/api/v1/signals?limit=5&minEdge=0.03`, z.array(SignalSchema));
        if (!signals || signals.length === 0) {
 ...
 */
async function handleChallenge(ctx, params) {
    try {
        const marketId = extractMarketId(params.market);
        validateMarketId(marketId);
        const analysis = await zigmaFetch(`/api/v1/market/${encodeURIComponent(marketId)}/analysis`, MarketAnalysisSchema);
